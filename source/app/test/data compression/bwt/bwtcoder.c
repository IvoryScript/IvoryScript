...
int size;
    int tcount = 0;
    unsigned char run[HUFF_size];
    int rIdx, zLen, zIdx, xFlag, zFlag;
   ...
   do
   {
      ...
      if (xFlag || idx == HUFF_size)
      {
         if (!zFlag)
         {
            ...
            count = idx - rIdx;
            mask = 1 << gamma_len(count);
            while (mask >>= 1 )
               arc_put1 (count & mask);

            for (; rIdx < idx; rIdx++)
            {
               count = HuffCode[rIdx].len;
               mask = 1 << gamma_len(count);
               while (mask >>= 1 )
                  arc_put1 (count & mask);
            }
...
unsigned code = HuffCode[val].code;
   ...
   printf("encoding %d, len = %d, code = %02x\n", val, HuffCode[val].len, code);
    while( mask >>= 1 )
        arc_put1 (code & mask);
...
void mtf_encode (unsigned char val, int flush,
                 unsigned idx, unsigned max, KeyPrefix *keys)
{
   static unsigned char mtf_prev;
   static unsigned mtf_count = 0;
   unsigned code;
   unsigned pos;
   ...
   if (mtf_count++ > 0)
   {
      code = (unsigned char *)memchr (MtfOrder, mtf_prev, 256) - MtfOrder;
      ...
      if (!SymFlag[mtf_prev])
      {
         ...
         {
            x = (double)SymFreq[mtf_prev] / (double)max;
            x = log(x);
      ...
      memmove (MtfOrder + pos + 1, MtfOrder + pos, code - pos);
      MtfOrder[pos] = mtf_prev;
...
char fred[20];
    ...
    memset (SymFlag, 0, sizeof(SymFlag));
    ZeroCnt = 0;
   ...
   for (idx = 0; idx < max; idx++)
    ...
    for( idx = 0; idx < max; idx++ )
      if( off = keys[idx].offset )
          mtf_encode (buff[--off], 0, idx, max, keys);
      else
          mtf_encode (MtfOrder[0], 0, idx, max, keys);

    mtf_encode (buff[max - 1], 0, idx, max, keys);
    mtf_encode (0, 1, max + 1, max, keys);
